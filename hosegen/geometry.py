...
def is_tetrahedral(mol, atom: Atom, wedgemap, strict=False):
    ...
    for bond in bonds:
        if (
            wedgemap[bond.GetIdx()] == 1
            and bond.GetBeginAtom().GetIdx() == atom.GetIdx()
        ):
            up += 1
        elif (
            wedgemap[bond.GetIdx()] == 6
            and bond.GetBeginAtom().GetIdx() == atom.GetIdx()
        ):
            down += 1
    if up == 1 and down == 1:
...
def order_stereo(gen, nodes, mol, chiralCenter, parent, wedgemap, strict=False):
    ...
    if parent is None:
        parent = Node(
            gen.get_element_symbol(chiralNeighbours[0].GetAtomicNum()),
            None,
            chiralNeighbours[0],
            0,
            chiralNeighbours[0].GetDegree() + chiralNeighbours[0].GetTotalNumHs(),
            0,
        )
    ...
    if is_tetrahedral(mol, chiralCenter, wedgemap, strict) == 1:
        if (
            wedgemap[
                mol.GetBondBetweenAtoms(
                    parent.atom.GetIdx(), chiralCenter.GetIdx()
                ).GetIdx()
            ]
            == 6
            and mol.GetBondBetweenAtoms(parent.atom.GetIdx(), chiralCenter.GetIdx())
            .GetBeginAtom()
            .GetIdx()
            == chiralCenter.GetIdx()
        ):
            ...
            for neighbour in chiralNeighbours:
                if neighbour.GetIdx() != parent.atom.GetIdx:
                    if (
                        wedgemap[
                            mol.GetBondBetweenAtoms(
                                neighbour.GetIdx(), chiralCenter.GetIdx()
                            ).GetIdx()
                        ]
                        == 0
    ...
    if is_tetrahedral(mol, chiralCenter, wedgemap, strict) == 2:
        if (
            wedgemap[
                mol.GetBondBetweenAtoms(parent.atom.GetIdx(), chiralCenter.GetIdx())
            ]
            == 1
            and mol.GetBondBetweenAtoms(parent.atom.GetIdx(), chiralCenter.GetIdx())
            .GetBeginAtom()
            ...
            for neighbour in chiralNeighbours:
                if neighbour.GetIdx() != parent.atom.GetIdx:
                    if (
                        ...
                        if angle1 == 0:
                            angle1 = give_angle(
                                mol, chiralCenter, parent.atom, neighbour
                            )
                            atom1 = neighbour
                        else:
                            angle2 = give_angle(
    ...
    if is_tetrahedral(mol, chiralCenter, wedgemap, strict) == 3:
        if (
            ...
            == 0
            or mol.GetBondBetweenAtoms(parent.atom.GetIdx(), chiralCenter.GetIdx())
            .GetBeginAtom()
            .GetIdx()
            != chiralCenter.GetIdx()
        ):
            ...
            for neighbour in chiralNeighbours:
                if neighbour.GetIdx() != parent.atom.GetIdx:
                    if (
                        ...
                        .GetIdx()
                        == chiralCenter.GetIdx()
                        and is_left(mol, neighbour, parent.atom, chiralCenter)
                    ):
                        sorted[0] = neighbour
                    if (
                        wedgemap[
                        ...
                        .GetIdx()
                        == chiralCenter.GetIdx()
                        and not is_left(mol, neighbour, parent.atom, chiralCenter)
                    ):
                        sorted[1] = neighbour
                    if (
                        wedgemap[
    ...
    if is_trigonal_bipyramidal_or_octahedral(mol, chiralCenter, wedgemap) != 0:
        ...
        if (
            ...
            for idx, neighbour in enumerate(chiralNeighbours):
                if (
                    wedgemap[
                        mol.GetBondBetweenAtoms(
                            chiralCenter.GetIdx(), neighbour.GetIdx()
                        ).GetIdx()
                    ]
                    == 0
                ):
        ...
        if (
            ...
            if numbyangle == 2:
                ...
                if (
                    give_angle_from_middle(
                        mol, chiralCenter, parent, chiralNeighbours[hm[1]]
                    )
                    < 0
                ):
...
def stereos_are_opposite(container, centre, wedgemap):
    ...
    stereoOne = wedgemap[
        container.GetBondBetweenAtoms(centre.GetIdx(), atoms[0].GetIdx()).GetIdx()
    ]
    stereoOpposite = wedgemap[
        container.GetBondBetweenAtoms(
            centre.GetIdx(), atoms[ohere[1]].GetIdx()
        ).GetIdx()
    ]
...
def is_square_planar(container, centre, wedgemap):
    ...
    for bond in bonds:
        if wedgemap[bond.GetIdx()] == 1:
            ...
            down += 1
    return (
        up == 2 and down == 2 and not stereos_are_opposite(container, centre, wedgemap)
    )
...
def create_wedgemap_from_block(molblock):
    ...
    for line in molblock:
        ...
        if (
            skip
            and count > atoms + 4
            and count < atoms + 5 + bonds
            and not "M  END" in line
        ):
    ...
    for a in container.GetAtoms():
        if len(a.GetNeighbors()) == 4:
            ...
            for conAtom in a.GetNeighbors():
                stereo = wedgemap[
                    container.GetBondBetweenAtoms(a.GetIdx(), conAtom.GetIdx()).GetIdx()
                ]
                if stereo == 1:
                    up += 1
                elif stereo == 6:
            ...
            if up == 0 and down == 1 and h is not None and hs == 1:
                wedgemap[
                    container.GetBondBetweenAtoms(a.GetIdx(), h.GetIdx()).GetIdx()
                ] = 1
