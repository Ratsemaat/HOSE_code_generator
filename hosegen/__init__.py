...
class HoseGenerator:
    ...
    def __init__(self) -> None:
        ...
        self.symbol_rankings: dict[str, int] = {
            "C": 9000,
            "O": 8900,
            "N": 8800,
            "S": 8700,
                ...
                for node2 in node_set:
                    ...
                    if node2.reorder is not None and tuple(node2.reorder) == reorder:
                        for idx, node3 in enumerate(reorder):
                            if not isinstance(node2.atom, Atom) or not isinstance(
                                node3.atom, Atom
                            ):
                                raise Exception("Bad state")
                            if node2.atom.GetIdx() == node3.atom.GetIdx():
                                node2.score = (
                                    reorder[count].score + len(reorder) - idx
                                )  # -idx is needed to distinguish identical atoms with different wedges
        ...
        for sphere in self.spheres:
            for node_set in self.spheres:
                for node in node_set:
                    ...
                    node.stringscore = str(node.connector.stringscore) + str(
                        node.score
                    ).zfill(6)
    ...
    def get_Hose_codes_from_file(
        ...
        if usestereo:
            ...
            return self.get_Hose_codes(
                self.mol,
                atom_idx,
                max_radius,
                usestereo,
                wedgebond=wedgebonds,
                strict=strict,
                ringsize=ringsize,
            )
        else:
    ...
    def get_Hose_codes(
        ...
        if usestereo:
            ...
            for bnd in bonds:
                if bnd.GetStereo() == BondStereo.STEREOE:
                    beginNbrs = [
                        x.GetIdx()
                        for x in bnd.GetBeginAtom().GetNeighbors()
                        if x.GetIdx() != bnd.GetEndAtomIdx()
                    ]
                    if len(beginNbrs) == 1:
                        beginNbrs.append(-1)
                    beginNbrs.extend(
                        [
                            x.GetIdx()
                            for x in bnd.GetEndAtom().GetNeighbors()
                            if x.GetIdx() != bnd.GetBeginAtomIdx()
                        ]
                    )
                    if len(beginNbrs) == 3:
                        beginNbrs.append(-1)
        ...
        root = Node(
            self.get_element_symbol(atom.GetAtomicNum()),
            None,
            atom,
            None,
        ...
        for i in range(max_radius):
            ...
            for node in current_layer:
                ...
                if len(bonds) == 1 and i > 0:
                    ...
                    for bond in bonds:
                        ...
                        if (
                            not isinstance(node.connector, Node)
                            or isinstance(node.connector.atom, Atom)
                            and to_atom.GetIdx() != node.connector.atom.GetIdx()
                        ):
                            interim_list.append(
                                Node(
                                    self.get_element_symbol(to_atom.GetAtomicNum()),
                                    node,
                                    to_atom,
                                    bond.GetBondType(),
                                    to_atom.GetDegree() + to_atom.GetTotalNumHs(),
                                    node.score,
                                )
                            )
            ...
            next_layer.sort(
                key=lambda x: x.atom.GetIdx() if x.atom else -sys.maxsize - 1,
                reverse=True,
            )
    ...
    def calculate_node_scores(self, sphere_nodes: list[Node]) -> None:
        ...
        for node in sphere_nodes:
            ...
            if (
                ...
                and isinstance(node.atom, Atom)
                and node.atom.HasProp("visited")
                and node.atom.GetProp("visited") == "true"
            ):
        ...
        for node in sphere_nodes:
            ...
            if isinstance(node.connector, Node) and not node.connector.stopper:
                ...
                if (
                    ...
                    and (
                        node.connector.atom.GetSymbol() == "C"
                        or node.connector.atom.GetSymbol() == "N"
                    )
                    and node.connector.bond_type == BondType.DOUBLE
                    and (
                        node.connector.connector.atom.GetSymbol() == "C"
                        or node.connector.connector.atom.GetSymbol() == "N"
                    )
                ):
                    if any(
                        x
                        for x in self.ezs
                        if (
                            x[0] == node.atom.GetIdx()
                            and x[2] == node.connector.connector.connector.atom.GetIdx()
                        )
                    ) or any(
                        x
                        for x in self.ezs
                        if (
                            x[1] == node.atom.GetIdx()
                            and x[3] == node.connector.connector.connector.atom.GetIdx()
                        )
                    ):
                        code += "\\"
                    if any(
                    ...
                    if any(
                        ...
                        for x in self.ezs
                        if (
                            x[0] == node.connector.connector.connector.atom.GetIdx()
                            and x[2] == node.atom.GetIdx()
                        )
                    ) or any(
                        x
                        for x in self.ezs
    ...
    def create_center_code(self, atom: Atom, mol: Mol, ringsize: int) -> None:
        ...
        self.center_code = (
            self.table.GetElementSymbol(atom.GetAtomicNum())
            + "-"
            + str(partners_count)
            + self.create_charge_code(atom)
            + (self.get_ringcode(atom, mol) * ringsize)
            + ";"
        )
